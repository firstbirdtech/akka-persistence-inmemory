...
class CurrentEventsByTagSequenceJournalTest extends QueryTestSpec {
  ...
  it should "find all events by tag" in {
    ...
    withCurrentEventsByTag()("number", NoOffset) { tp =>
      tp.request(Int.MaxValue)
      tp.expectNext(EventEnvelope(Sequence(1), "my-1", 1, "a-1", nowTs))
      tp.expectNext(EventEnvelope(Sequence(2), "my-2", 1, "a-1", nowTs))
      tp.expectNext(EventEnvelope(Sequence(3), "my-3", 1, "a-1", nowTs))
      tp.expectComplete()
    }
  ...
  it should "persist and find a tagged event with multiple tags" in {
    ...
    withCurrentEventsByTag()("prime", NoOffset) { tp =>
      tp.request(Int.MaxValue)
      tp.expectNext(EventEnvelope(Sequence(1), "my-1", 1, "a-1", nowTs))
      tp.expectNext(EventEnvelope(Sequence(2), "my-1", 2, "a-2", nowTs))
      tp.expectNext(EventEnvelope(Sequence(3), "my-1", 3, "a-3", nowTs))
      tp.expectNext(EventEnvelope(Sequence(4), "my-1", 5, "a-5", nowTs))
      tp.expectNext(EventEnvelope(Sequence(5), "my-2", 1, "a-1", nowTs))
      tp.expectNext(EventEnvelope(Sequence(6), "my-3", 1, "a-1", nowTs))
      tp.expectComplete()
    }
    ...
    withCurrentEventsByTag()("3", NoOffset) { tp =>
      tp.request(Int.MaxValue)
      tp.expectNext(EventEnvelope(Sequence(1), "my-1", 3, "a-3", nowTs))
      tp.expectNext(EventEnvelope(Sequence(2), "my-2", 1, "a-1", nowTs))
      tp.expectNext(EventEnvelope(Sequence(3), "my-3", 1, "a-1", nowTs))
      tp.expectComplete()
    }
    ...
    withCurrentEventsByTag()("4", NoOffset) { tp =>
      tp.request(Int.MaxValue)
      tp.expectNext(EventEnvelope(Sequence(1), "my-1", 4, "a-4", nowTs))
      tp.expectComplete()
    }
    ...
    withCurrentEventsByTag()("5", NoOffset) { tp =>
      tp.request(Int.MaxValue)
      tp.expectNext(EventEnvelope(Sequence(1), "my-1", 5, "a-5", nowTs))
      tp.expectComplete()
    }
