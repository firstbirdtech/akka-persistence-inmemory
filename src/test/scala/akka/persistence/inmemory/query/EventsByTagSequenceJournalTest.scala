...
class EventsByTagSequenceJournalTest extends QueryTestSpec {
  ...
  it should "find events for one tag starting with empty journal" in {
    withEventsByTag(10.seconds)("one", NoOffset) { tp =>
      ...
      tp.expectNext(EventEnvelope(Sequence(4), "my-1", 3, "a-3", nowTs))
      tp.expectNoMessage(NoMsgTime)

      persist(3, 3, "my-2", "one") // 8
      tp.expectNext(EventEnvelope(Sequence(5), "my-2", 3, "a-3", nowTs))
      tp.expectNoMessage(NoMsgTime)

      persist(3, 3, "my-3", "one") // 9
      tp.expectNext(EventEnvelope(Sequence(6), "my-3", 3, "a-3", nowTs))
      tp.expectNoMessage(NoMsgTime)
  ...
  it should "find events for one tag, starting with non-empty journal" in {
    ...
    withEventsByTag()("number", NoOffset) { tp =>
      ...
      tp.expectNext(EventEnvelope(Sequence(4), "my-1", 2, "a-2", nowTs))

      persist(3, 3, "my-1", "number") // 5
      tp.expectNext(EventEnvelope(Sequence(5), "my-1", 3, "a-3", nowTs))

      persist(4, 4, "my-1", "number") // 6
      tp.expectNext(EventEnvelope(Sequence(6), "my-1", 4, "a-4", nowTs))

      persist(2, 2, "my-2", "number") // 7
      tp.expectNext(EventEnvelope(Sequence(7), "my-2", 2, "a-2", nowTs))

      persist(2, 2, "my-3", "number") // 8
      tp.expectNext(EventEnvelope(Sequence(8), "my-3", 2, "a-2", nowTs))
