...
abstract class QueryTestSpec(config: String = "application.conf") extends TestSpec(config) {
  ...
  implicit lazy val defaultReadJournal: InMemoryReadJournal = PersistenceQuery(system).readJournalFor("inmemory-read-journal").asInstanceOf[InMemoryReadJournal]
  ...
  def withAllPersistenceIds(within: FiniteDuration = 10.seconds)(f: TestSubscriber.Probe[String] => Unit)(implicit readJournal: InMemoryReadJournal): Unit = {
