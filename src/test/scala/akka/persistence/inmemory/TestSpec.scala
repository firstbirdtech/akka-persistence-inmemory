...
abstract class TestSpec(config: Config)
    extends AnyFlatSpec
    with Matchers
    with ScalaFutures
    with Eventually
  ...
  val uuid_two_thousand_and_ten: TimeBasedUUID = getTimeBasedUUIDFromTimestamp(
    getTimestamp(DATE_FORMAT_TWO_THOUSAND_AND_TEN))
  val uuid_two_thousand_and_twenty: TimeBasedUUID = getTimeBasedUUIDFromTimestamp(
    getTimestamp(DATE_FORMAT_TWO_THOUSAND_AND_TWENTY))
