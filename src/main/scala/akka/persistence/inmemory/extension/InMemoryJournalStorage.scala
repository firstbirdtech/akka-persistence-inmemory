...
object InMemoryJournalStorage {
  ...
  final case class GetJournalEntriesExceptDeleted(persistenceId: String,
                                                  fromSequenceNr: Long,
                                                  toSequenceNr: Long,
                                                  max: Long)
      extends JournalCommand
  final case class GetAllJournalEntries(persistenceId: String, fromSequenceNr: Long, toSequenceNr: Long, max: Long)
      extends JournalCommand
...
class InMemoryJournalStorage(serialization: Serialization) extends Actor with ActorLogging {
  ...
  def delete(ref: ActorRef, persistenceId: String, toSequenceNr: Long): Unit = {
    ...
    val notDeleted = pidEntries.map { case (k, v) => (k, v.filterNot(_.sequenceNr <= toSequenceNr)) }

    val deleted = pidEntries
      .map { case (k, vs) =>
        (k,
         vs.filter(_.sequenceNr <= toSequenceNr).map { journalEntry =>
  ...
  override def receive: Receive = LoggingReceive {
    ...
    case GetAllJournalEntries(persistenceId, fromSequenceNr, toSequenceNr, max) =>
      messages(sender(), persistenceId, fromSequenceNr, toSequenceNr, max, all = true)
